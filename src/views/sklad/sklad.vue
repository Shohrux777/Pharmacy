...
export default {
  ...
  methods: {
    ...mapActions(['fetchSklad', 'fetchSklad_column']),
    ...mapMutations(['sklad_delete_row']),
    async checklist (){
        this.loading = true;
           await this.fetchSklad_column(this.auth_id)
        this.loading = false;
      },
      for_edit(edit_data)
      {
      ...
      async for_delete(del_data,index)
      {
         const requestOptions = {
          ...
          const response = await fetch(this.$store.state.hostname + "/PosSklads/" + del_data.id, requestOptions);
