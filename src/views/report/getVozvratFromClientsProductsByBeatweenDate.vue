    ...
    <div class="border-bottom d-flex justify-content-between">
      <router-link to="">
         <h5 class="m-0 ml-4 d-flex" style="padding: 16px 0px">
            {{$t('getVozvratFromClientsProductsByBeatweenDate')}}</h5>
      </router-link>
  ...
  methods: {
    ...
    pdf(){
      ...
      async refresh(){
        try{
          this.loading = true;
          const res = await fetch(this.$store.state.hostname + '/PosChecks/getReturnedProductsListByBeatweenDateForReport?page=' + this.activePage + '&size=' + this.value + '&begin_date=' + this.dateString + '&end_date=' + this.date1String+'&revert_status=3');
