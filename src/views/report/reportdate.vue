  ...
  <div>
    ...
    <div class="mx-3 py-1 p-0 border-bottom">
      <mdb-row>
        ...
        <mdb-col class="col-12 col-sm-6 col-md-4 col-lg-4 ">
        ...
        <mdb-col class="col-12 col-sm-6 col-md-3 col-lg-3 text-right">
...
export default {
  ...
  methods: {
    ...
    async srok(){
      var atime = new Date().toLocaleTimeString()
      console.log((atime))
      this.dateString = this.date + 'T'+ atime + '.000Z';
    ...
    pdf(){
      ...
      async refresh(){
        try{
          this.loading = true;
          const res = await fetch(this.$store.state.hostname + '/PosProductPrices/getSrokForProductList?page=' + this.activePage + '&size=' + this.value + '&srok_date_time=' + this.dateString);
  ...
  async mounted(){
    var atime = new Date().toLocaleTimeString()
    var adate = new Date().toLocaleDateString()
    this.date = adate.slice(6,10) + '-' + adate.slice(3,5) + '-' + adate.slice(0,2)
    this.dateString = adate.slice(6,10) + '-' + adate.slice(3,5) + '-' + adate.slice(0,2) + 'T'+ atime + '.000Z';
