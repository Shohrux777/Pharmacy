    ...
    <div class="border-bottom d-flex justify-content-between">
      <router-link to="">
         <h5 class="m-0 ml-4 d-flex" style="padding: 16px 0px">
            {{$t('getVozvratProductsByBeatweenDate')}}</h5>
      </router-link>
...
export default {
  ...
  methods: {
    ...
    async srok(){
      this.date = this.dateString + 'T00:00:00.000Z';
      this.date1 = this.date1String + 'T00:00:00.000Z';
      await this.refresh();
    ...
    pdf(){
      ...
      async refresh(){
        try{
          this.loading = true;
          const res = await fetch(this.$store.state.hostname + '/PosChecks/getReturnedProductsListByBeatweenDateForReport?page=' + this.activePage + '&size=' + this.value + '&begin_date=' + this.dateString + '&end_date=' + this.date1String+'&revert_status=2');
