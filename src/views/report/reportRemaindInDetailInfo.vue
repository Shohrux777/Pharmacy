  ...
  <div>
    ...
    <div class="mainChart px-3" id="print">
      <table  class="myTable">
        ...
        <tbody>
          <tr v-for="(item,i) in allreports" :key="i" :class="[{'bg-dark text-white': item.exp_days_count<=0},{'bg-danger text-white': item.exp_days_count > 0 && item.exp_days_count < 30},{'bg-warning text-white':  item.exp_days_count > 30 && item.exp_days_count < 60},{'bg-success text-white': item.exp_days_count > 60 && item.exp_days_count < 90}]">
            <td>{{i+1}}</td>
            <td>{{item.product_name}}</td>
            <td>{{item.real_qty.toFixed(2)}}</td>
            <td>{{item.unit_buyed_price}}</td>
            <td>{{item.created_date_str}} </td>
            <td>{{item.exp_date_str}} </td>
            <td>{{item.summ_of_remind}} </td>
          </tr>
        </tbody>
      </table>
    </div>
  ...
  methods: {
    ...
    pdf(){
      ...
      async refresh(){
        try{
          this.loading = true;
          const res = await fetch(this.$store.state.hostname + '/PosInvoiceItems/getOstatkaExpiredDateProductsList?page=' + this.activePage + '&size=' + this.value);
