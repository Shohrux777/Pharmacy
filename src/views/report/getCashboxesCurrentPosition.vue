    ...
    <div class="border-bottom d-flex justify-content-between">
      <router-link to="">
         <h5 class="m-0 ml-4 d-flex" style="padding: 16px 0px">
            {{$t('getCashboxesCurrentPosition')}}</h5>
      </router-link>
      ...
      <table  class="myTable">
        ...
        <tbody>
          <tr v-for="(item,i) in allreports" :key="i">
            ...
            <td>{{item.card_sum_str}}</td>
            <td>{{item.cash_sum_str}}</td>
            <td>{{item.click_sum_str}}</td>
            <td>{{item.online_sum_str}} </td>
            <td>{{item.humo_sum_str}}</td>
            <td>{{item.xarajat_sum_str}} </td>
            <td>{{item.total_sum_str}} </td>
          </tr>
        </tbody>
      </table>
    </div>
          ...
          <table  class="myTable">
               <thead>
                ...
                <tr v-for="(item,i) in allreports" :key="i">
                    ...
                    <td>{{item.card_sum}}</td>
                    <td>{{item.cash_sum}}</td>
                    <td>{{item.click_sum}}</td>
                    <td>{{item.online_sum}} </td>
                    <td>{{item.humo_sum}}</td>
                    <td>{{item.xarajat_sum}} </td>
  ...
  methods: {
    ...
    pdf(){
      ...
      async refresh(){
        try{
          this.loading = true;
          const res = await fetch(this.$store.state.hostname + '/PosChecks/getCurrentPositionOfAllCashboxByUserList?page=' + this.activePage + '&size=' + this.value);
