...
export default {
  ...
  validations: {
    ...
    async created()
    {
      if(this.id > 0)
      {
        const res = await fetch(this.$store.state.hostname + '/PosProductTypes/' + this.id);
  ...
  methods:{
    ...
    save_data :  async function(){
        ...
        const requestOptions = {
          ...
          try{
            this.loading = true;
            const response = await fetch(this.$store.state.hostname + "/PosProductTypes", requestOptions);
    ...
    async submit_continue()
      {
        ...
        {
          if(this.id != 0){
            this.$router.push('/productType_add/0')
            this.cls_wnd();
          }
          else{
            this.cls_wnd();
