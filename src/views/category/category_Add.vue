<template>
  <div>
    ...
    <div class="category_add p-5">
      <form @submit.prevent="submit">
        <mdb-row>
          <mdb-col col="3">
            <p class="p-0 m-0 mt-2" style="font-size: 14px;">{{$t('category_name')}}</p>
          </mdb-col>
          <mdb-col col="5">
...
export default {
  ...
  validations: {
    ...
    async created()
    {
      if(this.id > 0)
      {
        const res = await fetch(this.$store.state.hostname + '/PosCategories/' + this.id);
    ...
    async submit_continue()
      {
        ...
        {
          if(this.id != 0){
            this.$router.push('/category_add/0')
            this.cls_wnd();
          }
          else{
            this.cls_wnd();
