...
export default {
  ...
  methods: {
    ...mapActions(['fetchCategory', 'fetchCategory_column']),
    ...mapMutations(['category_delete_row']),
    async checklist (){
        this.loading = true;
           await this.fetchCategory_column(this.auth_id)
        this.loading = false;
      },
      for_edit(edit_data)
      {
      ...
      async for_delete(del_data,index)
      {
         const requestOptions = {
          ...
          const response = await fetch(this.$store.state.hostname + "/PosCategories/" + del_data.id, requestOptions);
