...
export default {
  ...
  methods: {
    ...mapActions(['fetchDepartment', 'fetchDepartment_column']),
    ...mapMutations(['department_delete_row']),
    async checklist (){
        this.loading = true;
           await this.fetchDepartment_column(this.auth_id)
        this.loading = false;
      },
      for_edit(edit_data)
      {
      ...
      async for_delete(del_data,index)
      {
         const requestOptions = {
          ...
          const response = await fetch(this.$store.state.hostname + "/PosDepartments/" + del_data.id, requestOptions);
          ...
          if(response.status == 201 || response.status == 200)
          {
            ...
            this.department_delete_row(index);
          }
          else{
