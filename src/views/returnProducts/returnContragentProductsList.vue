  ...
  <div>
    <div class="border-bottom d-flex justify-content-between">
      <router-link to="">
       ...
       <div>
         <router-link to="/retunContragent_add/0">
  ...
  methods: {
    ...
    pdf(){
      ...
      async refresh(){
        try{
          this.loading = true;
          const res = await fetch(this.$store.state.hostname + '/PosRevert/getPaginationKontragentgaQaytarilganTovarovList?page=' + this.activePage + '&size=' + this.value);
          const res_data = await res.json();
          console.log('res_data');
          console.log(res_data);
