<template>
  <div class="">
    ...
    <div class="border-bottom ">
      <router-link to="/retunContragent">
                ...
                <mdb-tbl class="fixed-column" >
                  ...
                  <mdb-tbl-body>
                    <tr  scope="row" v-for="(row,rowIndex) in datasource.rows" :key="rowIndex" :width="row.size">
                      ...
                      <th>
                        <input v-model="row.soni" type="number" min="0"  step=".1"  @input="onChangeCount(row.soni,row.unit,rowIndex)"
                      ...
                      <th> <input v-model="row.unit" @input="onChangeCount(row.soni,row.unit,rowIndex)" type="text" class="form-control form-control-sm">
                        <small class="invalid-text" style="margin:0; margin-top:26px; position:relative; "
                          v-if="($v.datasource.rows.$each[rowIndex].soni.$dirty && !$v.datasource.rows.$each[rowIndex].soni.minValue) ||
                          ($v.datasource.rows.$each[rowIndex].soni.$dirty && !$v.datasource.rows.$each[rowIndex].soni.required)" >
                            {{$t('minValue')}}
      ...
      async searchFunc(){
        if(this.searchDrag != ''){
            try{
              ...
              const response = await fetch(this.$store.state.hostname + "/PosInvoiceItems/getProductListForSaleByItemSearchByNameAndContragentId?name=" + this.searchDrag + '&res_count=500&pos_company_id='+this.company_id);
      ...
      async getbarcode(){
          ...
          if(this.barcode != ''){
            try{
              ...
              this.Arrowdown = 0;
              const response = await fetch(this.$store.state.hostname + "/PosInvoiceItems/getProductListForSaleByItemSearchByBarcodeAndContragentId?barcode=" + this.barcode + '&res_count=500&pos_company_id='+this.company_id);
              const data = await response.json();
            ...
            if(response.status == 200 || response.status == 201)
              {
                ...
                this.$router.push('/retunContragent')
              }
              else{
