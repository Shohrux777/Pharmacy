      ...
      async submit()
      {
        ...
        for(var i = 0; i < this.datasource.rows.length; i++){
          var a = {
            ...
            a.qty_in_pack = parseInt(this.datasource.rows[i].soni * this.datasource.rows[i].numberDrag) + parseInt(this.datasource.rows[i].unit)
            ...
            if(response.status == 200 || response.status == 201)
              {
                ...
                this.$router.push('/retunSpisaniya')
              }
              else{
...
.fixed-column {
  ...
  .tableProductsw{
    ...
    .myTablereturnS tr.header, .myTablereturnS tr:hover {
