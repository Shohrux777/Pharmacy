  ...
  <div>
    <div class="border-bottom d-flex justify-content-between">
      <router-link to="">
       ...
       <div>
         <router-link to="/retunSpisaniya_add/0">
  ...
  methods: {
    ...
    pdf(){
      ...
      async refresh(){
        try{
          this.loading = true;
          const res = await fetch(this.$store.state.hostname + '/PosRevert/getPaginationSpisatQilinganTovarovList?page=' + this.activePage + '&size=' + this.value);
          const res_data = await res.json();
          console.log('res_data');
          console.log(res_data);
