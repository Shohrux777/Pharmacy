...
export default {
  ...
  validations: {
    ...
    async created()
    {
      if(this.id > 0)
      {
        const res = await fetch(this.$store.state.hostname + '/PosProductUnitMeasurments/' + this.id);
    ...
    async submit_continue()
      {
        ...
        {
          if(this.id != 0){
            this.$router.push('/measurment_add/0')
            this.cls_wnd();
          }
          else{
            this.cls_wnd();
