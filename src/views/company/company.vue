...
export default {
  ...
  methods: {
    ...mapActions(['fetchCompany', 'fetchCompany_column']),
    ...mapMutations(['company_delete_row']),
    async checklist (){
        this.loading = true;
           await this.fetchCompany_column(this.auth_id)
        this.loading = false;
      },
      for_edit(edit_data)
      {
      ...
      async for_delete(del_data,index)
      {
         const requestOptions = {
          ...
          const response = await fetch(this.$store.state.hostname + "/PosCompanies/" + del_data.id, requestOptions);
