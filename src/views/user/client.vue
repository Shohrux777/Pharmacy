...
export default {
  ...
  methods: {
    ...mapActions(['fetchClient', 'fetchClient_column']),
    ...mapMutations(['client_delete_row']),
    async checklist (){
        this.loading = true;
           await this.fetchClient_column(this.auth_id)
        this.loading = false;
      },
      for_edit(edit_data)
      {
      ...
      async for_delete(del_data,index)
      {
         const requestOptions = {
          ...
          const response = await fetch(this.$store.state.hostname + "/PosClients/" + del_data.id, requestOptions);
