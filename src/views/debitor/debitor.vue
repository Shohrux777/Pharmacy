...
export default {
  ...
  methods: {
    ...
    async checklist (){
      ...
      for_edit(edit_data)
      {
        this.$router.push("/debitor_add/" + edit_data.id);
      },
      async for_delete(del_data,index)
      {
         const requestOptions = {
          ...
          const response = await fetch(this.$store.state.hostname + "/PosDebitorInvoice/" + del_data.id, requestOptions);
       ...
       async update_column()
      {
        const res = await fetch(this.$store.state.hostname + '/TexColumnConfigs/getColumnListAsJsonObj?tableName=pos_debitor_invoice&auth_id=' + this.auth_id);
      ...
      async refresh(){
        const res = await fetch(this.$store.state.hostname + '/PosDebitorInvoice/getPagination?page=' + this.get_pagination.page + '&size=' + this.get_pagination.size);
      ...
      async for_apply(id){
        ...
        console.log(id)
        try{
          const res = await fetch(this.$store.state.hostname + '/PosInvoices/applyOrDisableDebitorPosDebitorInvoice?invoice_id=' + id + '&apply_status=true');
      ...
      async for_cancel(id){
        ...
        console.log(id)
        try{
          const res = await fetch(this.$store.state.hostname + '/PosInvoices/applyOrDisableDebitorPosDebitorInvoice?invoice_id=' + id + '&apply_status=false');
          const data = await res.text();
          if(res.status == 201 || res.status == 200)
            {
              this.$refs.message.success('Added_successfully')
