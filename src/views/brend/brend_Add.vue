...
export default {
  ...
  validations: {
    ...
    async created()
    {
      if(this.id > 0)
      {
        const res = await fetch(this.$store.state.hostname + '/PosBrends/' + this.id);
  ...
  methods:{
    ...
    save_data :  async function(){
        ...
        const requestOptions = {
          ...
          try{
            this.loading = true;
            const response = await fetch(this.$store.state.hostname + "/PosBrends", requestOptions);
