...
export default {
  ...
  methods: {
    ...mapActions(['fetchBrend', 'fetchBrend_column']),
    ...mapMutations(['brend_delete_row']),
    async checklist (){
        this.loading = true;
           await this.fetchBrend_column(this.auth_id)
        this.loading = false;
      },
      for_edit(edit_data)
      {
        this.$router.push("/brend_add/" + edit_data.id);
      },
      async for_delete(del_data,index)
      {
         const requestOptions = {
          ...
          const response = await fetch(this.$store.state.hostname + "/PosBrends/" + del_data.id, requestOptions);
