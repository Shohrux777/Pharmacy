    ...
    <div class="mainChart px-3" id="print">
      <table  class="myTable">
        <thead>
          ...
          <th style="width: 300px;">{{$t('postavshik')}}</th>
          <th>{{$t('credit_sum')}}</th>
        ...
        <tbody>
          <tr v-for="(item,i) in allreports" :key="i" >
            <td>{{i+1}}</td>
            <td>{{item.postavshik_name}}</td>
            <td>{{item.creditor_price_str}}</td>
            <td>{{item.real_creditor_price_str}}</td>
      ...
      <div slot="pdf-content">
        ...
        <tbody>
          <tr v-for="(item,i) in allreports" :key="i" >
            <td>{{i+1}}</td>
            <td>{{item.postavshik_name}}</td>
            <td>{{item.creditor_price}}</td>
            <td>{{item.real_creditor_price}}</td>
            <td>{{item.reg_date_str}} </td>
          </tr>
        </tbody>
          </table>
  ...
  methods: {
    ...
    pdf(){
      ...
      async refresh(){
        try{
          this.loading = true;
          const res = await fetch(this.$store.state.hostname + '/PosCreditor/getPaginationPosCreditor?page=' + this.activePage + '&size=' + this.value);
