                ...
                <mdb-tbl class="fixed-column" >
                  <mdb-tbl-head>
                    <tr>
                      ...
                      <td style="min-width:100px; font-size: 13px;"> {{$t(datasource.columns[7])}} </td>
                      <td style="min-width:100px; font-size: 13px;"> {{$t(datasource.columns[8])}} </td>
                      <td style="min-width:100px; font-size: 13px;"> {{$t(datasource.columns[9])}} </td>
                      <td style="min-width:100px; font-size: 13px;"> {{$t(datasource.columns[10])}} </td>
                    ...
                    <tr  scope="row" v-for="(row,rowIndex) in datasource.rows" :key="rowIndex" :width="row.size">
                      ...
                      <th>
                        <lineSelect
                            ...
                            @addTag="add_product"
                            :label="$t('product')"
                            url="/PosProducts/checkProductByNameList?name="
                        />
                      ...
                      <th> <input v-model="row.unit" type="text" class="form-control form-control-sm"> </th>
                      <th> <input  v-model="row.numberDrag" disabled type="text" class="form-control form-control-sm"> </th>
                      <th> <input v-model="row.price" @input="onChangePrice(rowIndex)" type="text" class="form-control form-control-sm"> </th>
                      <th> <input v-model="row.prosent" @input="onchange_presentage(rowIndex)" type="number" class="form-control form-control-sm" >  </th>
                      <th> <input v-model="row.buyed_price" @input="onchange_buyed(rowIndex)" type="text" class="form-control form-control-sm" >  </th>
                      <th> <input v-model="row.prosentNDS" @input="onchange_presentageNDS(rowIndex)" type="number" class="form-control form-control-sm" >  </th>
                      <th> <input v-model="row.persantage_discount"  type="number" class="form-control form-control-sm" >  </th>
...
import lineSelect from "../../components/lineSelectSearch.vue";
    ...
    computed:mapGetters(['allSklad', 'allDepartment', 'allCompany', 'allProduct']),
     ...
     async close_product(last_product)
      ...
      onchange_buyed(index){
         var p = parseInt(this.datasource.rows[index].buyed_price) - parseInt(this.datasource.rows[index].price)
        this.datasource.rows[index].prosent = (p*100)/this.datasource.rows[index].price;
      },
      async getbarcode(){
        if(this.barcode != '')
        {
          try{
            const response = await fetch(this.$store.state.hostname + "/PosProducts/checkProductByBarcode?barcode=" + this.barcode);
            const data = await response.json();
            console.log('data kelgan object')
            console.log(data)
            if(data.id){
              this.sum = 0;
                for(let j=0; j<this.datasource.rows.length; j++){
                  if(data.id == this.datasource.rows[j].product_id){
                    await this.sum++;
                    break;
                  }
                }
                if(this.sum == 0){
                    console.log('natija')
                    // let newDate = new Date().getFullYear()+3;
                    // let YearString = newDate.toString().slice(2,4)
                  ...
                  let i = 0;
                  this.datasource.rows[i].product_id = data.id
                  this.datasource.rows[i].product_name = data.name
                  this.datasource.rows[i].buyed_price = data.buyed_price
                  this.datasource.rows[i].price = data.price
                  this.datasource.rows[i].numberDrag = data.contains_number_in_pack;
                  this.datasource.rows[i].buyed_price = data.buyed_price;
                  this.datasource.rows[i].price = data.price;
                  this.datasource.rows[i].prosent = data.percentage;
                  this.datasource.rows[i].soni = 0;
                  this.datasource.rows[i].prosentNDS = 0;
                  this.datasource.rows[i].persantage_discount = 0;
                  this.datasource.rows[i].unit = 0;
                  this.barcode = '';
                }
            }
          }
      ...
      async submit()
      {
        ...
        for(var i = 0; i < this.datasource.rows.length; i++){
          var a = {
            ...
            change_price_status: false

          }
          a.expired_date = year.toString().slice(0,2) + this.datasource.rows[i].date.slice(3,5)+ '-' + this.datasource.rows[i].date.slice(0,2)+ '-01T00:00:00.014Z'
          ...
          a.real_qty = a.qty;
          a.summ = this.datasource.rows[i].price * this.datasource.rows[i].soni;
          a.persantage_nds = this.datasource.rows[i].prosentNDS;
          a.persantage = this.datasource.rows[i].prosent;
          a.qty_in_pack = parseInt(this.datasource.rows[i].soni)*parseInt(this.datasource.rows[i].numberDrag)+parseInt(this.datasource.rows[i].unit);
          a.saved_in_pack = a.qty_in_pack;
          a.persantage_discount = this.datasource.rows[i].persantage_discount;
          a.unit_buyed_price = parseFloat(this.datasource.rows[i].price);
          a.unit_saled_price = parseFloat(this.datasource.rows[i].buyed_price);
          a.id = this.datasource.rows[i].id;
          a.change_price_status = this.datasource.rows[i].change_price_status;
          ...
          try{
            ...
            for(let k = 0; k < this.datasource.rows.length; k++){
              swq += this.datasource.rows[k].price * this.datasource.rows[k].soni
            }
